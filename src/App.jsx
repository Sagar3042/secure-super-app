...
export default function SuperApp() {
  ...
  if (loading) return <div className="flex h-screen items-center justify-center bg-gray-900 text-white">Loading App...</div>;
      ...
      <div className="bg-gray-800 p-4 flex items-center justify-between border-b border-gray-700 z-10">
        <div className="flex items-center space-x-3">
          {currentScreen !== 'dashboard' && (
            <button onClick={() => setCurrentScreen('dashboard')} className="text-blue-400 hover:text-blue-300">
              Back
            </button>
          )}
          {currentScreen === 'dashboard' && (
        ...
        <div className="flex items-center space-x-3">
          ...
          {profile?.isPremium ? (
            <span className="bg-gradient-to-r from-yellow-400 to-yellow-600 text-black text-xs font-bold px-2 py-1 rounded flex items-center">
              <Lock size={12} className="mr-1" /> Premium
            </span>
          ) : (
            <button onClick={() => setCurrentScreen('referral')} className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded">
              Get Premium
            </button>
          )}
        </div>
  ...
  const handleSave = async () => {
    ...
    if (file) {
      ...
      if (uploadResult && uploadResult.url) {
        dpUrl = uploadResult.url;
      } else {
        alert("Photo upload e somossa hoyeche, default photo use kora hocche.");
      }
    }
    ...
    try {
      if (referCode) {
        ...
        if (referrer) {
          ...
          await updateDoc(doc(db, 'artifacts', appId, 'public', 'users', referrer.id), { 
            ...
            isPremium: newCount >= 5 || referrer.isPremium
          });
        }
      }
      ...
      console.error(err);
      alert("Profile save korte somossa hoyeche.");
    }
    setLoading(false);
  };
...
function Dashboard({ setScreen, profile }) {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-4 text-gray-300">Features</h2>
      <div className="grid grid-cols-2 gap-4">
        {tools.map(tool => (
          <div 
            key={tool.id} 
            onClick={() => setScreen(tool.id)}
            className="bg-gray-800 border border-gray-700 p-4 rounded-xl flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-750 transition active:scale-95"
          >
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 text-white ${tool.color}`}>
              {tool.icon}
            </div>
        ...
        {profile?.isAdmin && (
          <div 
            ...
            className="bg-red-900/30 border border-red-700 p-4 rounded-xl flex flex-col items-center justify-center text-center cursor-pointer transition active:scale-95"
          >
            <div className="w-14 h-14 rounded-full flex items-center justify-center mb-3 text-white bg-red-600">
              <ShieldAlert size={28} />
            </div>
            <h3 className="font-bold text-sm text-red-400">Admin Panel</h3>
            <p className="text-xs text-gray-400 mt-1">Send Notifications</p>
          </div>
        )}
      </div>
    </div>
...
function AdminScreen({ user, profile }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      />
      <button onClick={sendNotif} disabled={loading} className="bg-red-600 font-bold p-3 rounded-lg text-white">
      ...
      <div className="space-y-3">
        {notifications.map(n => (
          <div key={n.id} className="bg-gray-800 p-4 rounded-lg border-l-4 border-blue-500">
            ...
            <p className="text-xs text-gray-500 mt-2">By Admin: {n.sender}</p>
          </div>
        ))}
      </div>
    </div>
...
function ReferralScreen({ profile }) {
  const copyCode = () => {
    ...
    alert("Refer Code Copy kora hoyeche!");
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center pt-10">
      <div className="bg-gray-800 p-6 rounded-2xl w-full border border-gray-700 text-center">
        ...
        <h2 className="text-2xl font-bold mb-2 text-white">Get Premium</h2>
        <p className="text-sm text-gray-400 mb-6">Premium features unlock korte 5 jon bondhu ke refer korun.</p>
        ...
        {profile?.isPremium ? (
          ...
          <p className="text-xs text-yellow-500">Aro {5 - (profile?.referralCount || 0)} refer lagbe.</p>
        )}
      </div>
    </div>
  );
...
function AnonChatScreen({ user, usersList }) {
          ...
          {usersList.filter(u => u.id !== user.uid).map(u => (
            <div key={u.id} onClick={() => setSelectedUser(u)} className="bg-gray-800 p-3 rounded flex items-center space-x-3 cursor-pointer hover:bg-gray-700">
               ...
               <div>
                 ...
                 <div className="text-xs text-gray-400">Start private chat</div>
               </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col h-[80vh]">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 p-2 bg-gray-900 rounded custom-scrollbar border border-gray-800">
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.senderId === user.uid ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] p-3 rounded-lg text-sm ${m.senderId === user.uid ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-100 rounded-bl-none'}`}>
              {m.text}
            </div>
          </div>
        ))}
      ...
      <div className="mt-4 flex space-x-2">
        <input 
          ...
          className="flex-1 bg-gray-800 rounded p-3 text-white border border-gray-700 outline-none focus:border-blue-500" placeholder="Message..."
        />
...
function ViewOnceScreen({ user, usersList }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (result && result.url) {
      try {
        ...
        alert("Secret Photo Pathano Hoyeche!");
      } catch (err) {
        console.error(err);
        alert("Database error.");
      }
    } else {
      alert("ImageKit a upload fail hoyeche. API Key check korun.");
    }
    setUploading(false);
  };
  ...
  const handleReleaseView = () => {
    ...
    if (viewingImg) deletePhoto(viewingImg.id);
  };

  const deletePhoto = async (id) => {
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'viewOnce', id));
  ...
  if (viewingImg) {
    return (
      <div 
        ...
        onMouseUp={handleReleaseView} onTouchEnd={handleReleaseView} onMouseLeave={handleReleaseView}
      >
        <div className="absolute top-10 text-white font-bold text-xl flex items-center bg-red-600 px-4 py-2 rounded-full shadow-lg">
          <EyeOff className="mr-2" /> Deleting in {timeLeft}s
        </div>
        <p className="absolute top-24 text-gray-400 text-xs text-center px-6">Release to delete immediately.</p>
        <img 
          src={viewingImg.imgUrl} 
          alt="Secure" 
  ...
  return (
    <div>
      ...
      <div className="bg-gray-800 p-4 rounded-xl mb-6 border border-gray-700">
        <select className="w-full bg-gray-900 text-white p-3 rounded mb-4 border border-gray-700 outline-none"
        ...
        {images.map(img => {
          ...
          return (
            <div key={img.id} className="bg-gray-800 border border-red-500/30 p-4 rounded-lg flex items-center justify-between">
              <div>
                <span className="font-bold text-sm text-red-400">Secret Photo</span>
                <p className="text-xs text-gray-400">From: {sender?.name || 'Unknown'}</p>
              </div>
              <button 
                onMouseDown={() => handleHoldView(img)} onTouchStart={() => handleHoldView(img)}
                className="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded text-sm font-bold active:bg-gray-500 transition shadow-lg select-none touch-none"
              >
                Hold to View
              </button>
            </div>
...
function BrowserScreen() {
  ...
  return (
    <div className="flex flex-col h-[80vh]">
      <form onSubmit={go} className="flex mb-4 space-x-2">
        <input type="text" value={inputUrl} onChange={e=>setInputUrl(e.target.value)} placeholder="Website URL" className="flex-1 bg-gray-800 p-2 rounded text-sm border border-gray-700 outline-none text-white"/>
        <button type="submit" className="bg-teal-600 px-4 py-2 rounded text-sm font-bold">Go</button>
      </form>
      <div className="flex-1 bg-white rounded overflow-hidden">
         <iframe src={url} className="w-full h-full border-none" title="Browser"></iframe>
      </div>
    </div>
  );
}
...
function DocReaderScreen() {
  ...
  return (
    <div className="flex flex-col h-[80vh]">
      <h2 className="text-lg font-bold mb-4">Read PDF / Excel / Word</h2>
      <label className="w-full bg-gray-800 border-2 border-dashed border-gray-600 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-orange-500 transition mb-4">
        <FileText size={32} className="text-orange-500 mb-2" />
        <span className="text-sm font-bold text-gray-300">Select File</span>
        <input type="file" accept=".pdf,.txt,.csv,.docx,.xlsx" className="hidden" onChange={handleFile} />
      </label>
      {fileUrl && (
        <div className="flex-1 flex flex-col bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
           <div className="bg-gray-900 p-2 text-xs font-mono text-gray-400 border-b border-gray-700 truncate">{fileName}</div>
           <object data={fileUrl} className="w-full h-full bg-white">
             <div className="p-4 text-black text-center mt-10">
                <p>Eita app er bhitor open hoyni (Android limitation).</p>
                <a href={fileUrl} download={fileName} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded inline-block">Download & View</a>
             </div>
           </object>
        </div>
      )}
